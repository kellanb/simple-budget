...
export function YearlyLineItemRow({
  ...
  // Helper for truncated text cells
  const renderTruncated = (text: string, maxWidth = "max-w-[180px]") => {
    if (!text || text === "—") return text;
    return <TruncatedText text={text} maxWidth={maxWidth} />;
  };
  ...
  const renderCellValue = (columnKey: string) => {
    ...
    // Handle regular columns - use truncation for text fields
    switch (columnKey) {
      case "label":
        return renderTruncated(item.label, "max-w-[160px]");
      case "amountCents":
        return formatCurrency(item.amountCents);
      case "dueDate":
        return renderTruncated(item.dueDate || "—", "max-w-[120px]");
      case "paymentSource":
        return renderTruncated(item.paymentSource || "—", "max-w-[120px]");
